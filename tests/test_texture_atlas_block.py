...
def test_sprite_atlas_replacement_blocked(tmp_path, caplog):
    ...
    (skin / "assets" / "icons" / "mapping.json").write_text(
        json.dumps({"my_icon": "AtlasSpriteA"}), encoding="utf-8"
    )
    ...
    sprite_a = SimpleNamespace(
        m_Name="AtlasSpriteA",
        m_RD=SimpleNamespace(texture=SimpleNamespace(m_PathID=9009)),
    )
    sprite_b = SimpleNamespace(
        m_Name="AtlasSpriteB",
        m_RD=SimpleNamespace(texture=SimpleNamespace(m_PathID=9009)),
    )
