...
def create_circle_mesh(radius: float = 0.5, segments: int = 48) -> Tuple[List[Tuple[float, float, float]], List[int]]:
...
def svg_path_to_mesh(svg_path: str, *, samples: int = 96) -> Optional[Tuple[List[Tuple[float, float, float]], List[int]]]:
    ...
    try:
        ...
        for p in points:
            if not deduped or (abs(deduped[-1][0] - p[0]) > 1e-5 or abs(deduped[-1][1] - p[1]) > 1e-5):
                deduped.append(p)
...
def replace_vector_sprite(sprite_obj, shape: str = "circle", color: Optional[Tuple[int, int, int, int]] = None, **kwargs):
    ...
    try:
        ...
        if not indices:
            log.warning(
                "[VECTOR] Mesh for '%s' produced no indices", sprite_obj.m_Name)
            return False
